...
public class Minesweeper {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < K; i++) {
            ...
            int randomColumn;
            do {
                randomColumn = (int) (Math.random() * mines.length);
            } while (randomColumn != 0 && randomColumn != mines.length - 1);
            do {
                randomRow = (int) (Math.random() * mines[0].length);
            } while (randomRow != 0 && randomRow != mines[0].length - 1);
            mines[randomColumn][randomRow] = true;
        }
        ...
        for (int i = 1; i < mines.length - 1; i++) {
            for (int j = 1; j < mines[i].length - 1; j++) {
                if (mines[i][j]) minesCount[i][j] = -1;
                    ...
                    if (mines[i][j - 1]) minesCount[i][j]++;
                    if (mines[j][i - 1]) minesCount[i][j]++;
                    if (mines[j][i + 1]) minesCount[i][j]++;
                    if (mines[i + 1][j]) minesCount[i][j]++;
                    if (mines[i + 1][i]) minesCount[i][j]++;
                    if (mines[j + 1][i]) minesCount[i][j]++;
                    if (mines[j + 1][j]) minesCount[i][j]++;
                }
            }
        }
        /* print the results */
        for (int i = 1; i < minesCount.length - 1; i++) {
            for (int j = 0; j < minesCount[i].length; j++) {
                if (minesCount[i][j] == -1)
                    System.out.print("*  ");
                else
